...
{
	// multi-oom을 위한 fd 조건
	if (thread_current()->last_created_fd == 128) {
		exit(128);
	}
...
int write (int fd, const void *buffer, unsigned size)
{
	...
	for (start = list_begin(&t->fd_table); start != list_end(&t->fd_table); start = list_next(start))
	{
...
void exist_address(const uint64_t *buffer) {
	struct thread *cur = thread_current();
	if (buffer == NULL || !is_user_vaddr(buffer) || pml4_get_page(cur->pml4, buffer) == NULL) {
		exit(-1);
	}
}